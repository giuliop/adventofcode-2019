...
(defn inside? [p [p1 p2]]
  "Takes a point and a segment and returns whether the point lies on the
  segment"
  (let [[x y] p, [x1 y1] p1, [x2 y2] p2]
    (or (and (= x x1 x2) (or (<= y1 y y2) (<= y2 y y1)))
        (and (= y y1 y2) (or (<= x1 x x2) (<= x2 x x1))))))

(defn steps [intersections segments]
  "Takes a set of points and a list of segments and returns a list of
  the # of steps the wire takes to reach each point"
  (map (fn step-to [intersection]
         (loop [segments segments
                res 0]
           (let [[p1 p2] (first segments)]
             (if (inside? intersection [p1 p2])
               (+ res (distance p1 intersection))
               (recur (rest segments) (+ res (distance p1 p2)))))))
       intersections))

(defn answer []
  "Return the quiz answer"
  (let [intersections (vec (apply intersections (inputs)))
        [steps-wire1 steps-wire2] (map #(steps intersections %)
                                       (map segments (inputs)))]
    (apply min (mapv + steps-wire1 steps-wire2))))
