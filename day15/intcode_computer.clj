...
(defn run
  ([input memory]
   "Run with default value pc 0, base 0, end? true"
   (run input memory 0 0 true))

  ([input memory pc base end?]
   "Run the program in memory starting at program counter pc and base
   with the given input list. Returns [output state] as the following:
   [[outputs] [[inputs] [memory] program-counter base ended?]]

   If end? returns when program halts, otherwise when it emits output
   If an input is asked but input is empty returns (so it can be provided)"
   ...
   (loop [pc pc
     ...
     (let [[opcode p1 p2 p3] (opcode-and-params memory pc base)]
       ...
       ;(println)
       (case opcode
         99 [output [input memory pc base true]]
         ; 3 read an input and store it at address p1
         3 (if (empty? input) [output [input memory pc base false]]
             (recur (+ pc 2) base
         ...
         4 (let [output (conj output p1)]
             (if end? (recur (+ pc 2) base memory input output)
               [output [input memory (+ pc 2) base false]]))
